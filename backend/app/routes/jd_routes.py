...
@router.get('/jd/stop_spider/{task_id}')
...
@router.get('/jd/resume_spider/{task_id}')
async def resume_spider(task_id: str, current_user: User = Depends(get_current_user)):
    ...
    if task_type == "jd_product":
        task = celery.send_task('tasks.run_jd_product_spider', task_id=task_id, kwargs=task_kwargs)
    elif task_type == "jd_comment":
        task = celery.send_task('tasks.run_jd_comment_spider', task_id=task_id, kwargs=task_kwargs)

    task_db["tasks"].update_one({"task_id": task_id, "user_id": current_user.id},
                                {"$set": {"status": "running"}})

    return {"message": "Task has been resumed."}
